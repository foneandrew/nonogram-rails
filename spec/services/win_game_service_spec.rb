...
RSpec.describe WinGameService, :type => :service do
  ...
  describe '#call' do
    context 'when the game is not finshed' do
      ...
      context 'when the answer is correct' do
        ...
        let(:win_game_service) { WinGameService.new(game: game, answer: answer) }

        it 'returns true' do
          expect(win_game_service.call).to be_truthy
        end

        it 'sets the finish time in the game' do
          expect{
            win_game_service.call
      ...
      context 'when the answer is wrong' do
        context 'when the answer is off by one at the start of string' do
          ...
          it 'returns false' do
            expect(win_game_service.call).to be_falsey
          end
